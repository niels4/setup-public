...
type ReadPasswordFunc = (prompt?: string) => Promise<string>

const savePassSecret = async (key: string, secret: string) => {
  await shell(`echo "${secret}" | pass insert -m -f ${key}`)
}
...
const initGpg = async (readPassword: ReadPasswordFunc) => {
  ...
  const passphrase = await readPassword("Enter password to save ssh passphrase to keyring: ")
...
type LinuxSshSetupOptions = {
  mirrorUserPassword?: boolean
}

export default async function setup(options: LinuxSshSetupOptions = {}) {
  ...
  const readPassword: ReadPasswordFunc = options.mirrorUserPassword ? readUserPassword : readNewPassword

  await initGpg(readPassword)
  await initPass()
  await initSshKey()
